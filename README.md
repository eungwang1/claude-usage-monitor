...
"항상 허용"을 눌러도 창이 반복해서 뜨면 한 번만 실행하세요:

```bash
./fix-keychain-prompts.sh
```

macOS Sierra부터 키체인 항목에는 ACL 외에 **파티션 목록**이라는 2차 관문이 있습니다. "항상 허용"은 ACL에만 기록되기 때문에, 파티션 목록이 비어 있으면 다음 접근에서 또 묻습니다 — 몇 번을 눌러도 마찬가지입니다. 이 스크립트가 파티션 목록을 직접 채웁니다. 키체인 암호를 한 번 묻고, 그 뒤로는 뜨지 않습니다.

`apple-tool:` 항목이 필요한 이유는 **Claude Code가 `/usr/bin/security` 명령을 통해 키체인에 접근**하기 때문입니다. 그래서 허용 창의 요청 주체가 앱 이름이 아니라 `security`로 표시됩니다. 이 앱은 서명 팀 ID로 식별되며, 스크립트가 `codesign`으로 자동 추출합니다.
...
2. 표시 이름(예: `회사`, `개인A`)을 입력하고 **가져오기**
   - 처음 한 번은 macOS가 키체인 접근 허용을 묻습니다. **암호를 입력하고** "항상 허용"을 누르세요 (빈칸으로 누르면 저장되지 않습니다). 그래도 반복되면 위의 [키체인 허용 창이 계속 뜬다면](#키체인-허용-창이-계속-뜬다면)을 실행하세요.
